...
export default function OrderDetailsDrawer({ open, onClose, orderData }) {
    const { user } = useAuth();
    const { updateOrderStatus, cancelOrder, orders } =
    ...
    const flaggedItems = orderData?.products?.filter(
        (product) =>
            ...
            (product.inventoryId?.isLocal && !orderData.userId?.isLocal)
    );
    const hasLocalRestriction = flaggedItems?.length > 0;

    React.useEffect(() => {
        const getUserOrders = async () => {
            const userOrder = await orders.filter(
                (o) => o.userId._id === orderData?.userId?._id
            );
            setTotalUserOrder(userOrder);

            const detectedAddress = await getUserLatestLocation(
                orderData?.userId?._id
            );
            const address = `${
                detectedAddress?.latest?.fullAddress?.address?.state
            }, ${detectedAddress?.latest?.fullAddress?.address?.region}, ${
                detectedAddress?.latest?.fullAddress?.address?.postcode
            }, ${detectedAddress?.latest?.fullAddress?.address?.country_code.toUpperCase()}`;
    ...
    const handleClick = async ({ orderId }) => {
        ...
        try {
            ...
            if (success) {
                ...
                console.log(
                    `DB ORDER(${orderId}) STATUS UPDATED TO: ${current.next}`
                );
            } else {
            ...
            >
                ...
                <SectionWrapper sx={{ gap: 3 }}>
                    <Stack>
                        ...
                        <Typography variant="body2" color="gray">
                            ...
                            is "<b>{orderData.purchaseStatus.toUpperCase()}</b>"
                        </Typography>
                    </Stack>
                    ...
                    orderData.purchaseStatus === "cancelled" ? (
                        ...
                        <Stack gap={2}>
                            <Stack>
                                ...
                                <Typography variant="body2" color="gray">
                                    Manage order lifecycle including status
                                    updates, cancellation, and print records
                                </Typography>
                            </Stack>
                            ...
                            <Stack direction={"row"} gap={1}>
                                <Button
                                    ...
                                    sx={{ py: 1 }}
                                    onClick={() =>
                                        handleClick({ orderId: orderData._id })
                                    }
                                ...
                                >
                                    {loading ? (
                                        ...
                                        <Typography variant="body" color="grey">
                                            Please wait 10 seconds for another
                                            update...
                                        </Typography>
                                    ) : (
                                        <Stack
                                ...
                                {(orderData?.purchaseStatus === "confirmed" ||
                                    orderData?.purchaseStatus ===
                                        "processing") && (
                                    <Stack
                                        direction={"row"}
                                        gap={1}
                            ...
                            {(orderData?.purchaseStatus === "pending" ||
                                orderData?.purchaseStatus === "confirmed") && (
                                <Stack gap={1}>
                                    <Stack
                                        direction={"row"}
                ...
                <SectionWrapper>
                    <Stack gap={2}>
                        ...
                        >
                            ...
                            <Typography variant="body2" color="secondary">
                                {toTitleCase(orderData?.orderType)} order
                            </Typography>
                        </Stack>
                        <Stack
                            direction={"row"}
                        ...
                        >
                            ...
                            <Typography variant="body2" color="secondary">
                                Php {formatNumber(orderData?.totalPrice)}
                            </Typography>
                        </Stack>
                        <Stack
                            direction={"row"}
                        ...
                        >
                            ...
                            <Typography variant="body2" color="secondary">
                                Php {formatNumber(orderData?.finalPrice)}
                            </Typography>
                        </Stack>
                        <Stack
                            direction={"row"}
                    ...
                    <Stack gap={3}>
                        ...
                        <Stack gap={2}>
                            ...
                            >
                                ...
                                >
                                    <MapIcon /> Local/International :
                                </Typography>
                                <Typography variant="body2" color="secondary">
                                    {orderData.userId.isLocal
                                        ? "Local"
                                        : "International"}
                                </Typography>
                            </Stack>
                            ...
                            >
                                ...
                                >
                                    <ContactIcon /> Member Since :{" "}
                                </Typography>
                                <Typography variant="body2" color="secondary">
                                    {dayjs(orderData?.userId.createdAt).format(
                                        "MMMM YYYY"
                                    )}
                                </Typography>
                            </Stack>
                            ...
                            >
                                ...
                                >
                                    <BoxIcon /> Previous Orders :{" "}
                                </Typography>
                                <Typography variant="body2" color="secondary">
                                    {totalUserOrder.length}
                                </Typography>
                            </Stack>
                            <Stack
                                sx={{
                            ...
                            >
                                <Stack justifyContent={"space-between"}>
                                    ...
                                    >
                                        <MapPinHouseIcon /> Shipping Address :{" "}
                                    </Typography>
                                    <Typography
                                        variant="body2"
                                        color="secondary"
                            ...
                            <>
                                <Stack justifyContent={"space-between"}>
                                    ...
                                    <Typography
                                        variant="body2"
                                        color="warning"
                                        sx={{ bgcolor: "#ff910036", p: 1 }}
                                    >
                                        User order includes large item which
                                        can't be available for shipment
                                    </Typography>
                                </Stack>
                            </>
                        )}
                ...
                <SectionWrapper sx={{ gap: 2 }}>
                    <Typography variant="subtitle2" color="secondary">
                        Items ordered ({orderData?.products.length})
                    </Typography>
