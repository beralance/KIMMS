...
const Orders = () => {
    ...
    useEffect(() => {
        const filtered = orders.filter((o) => {
            try {
                const filtered = orders.filter((o) => {
                    if (
                        (o.isActive && o.orderStatus === "SUCCESSFUL") ||
                        (o.isActive &&
                            o.orderStatus === "CANCELLED" &&
                            o.purchaseStatus === "cancelled")
                    ) {
                        return true;
                    } else {
    ...
    const handleSearch = async () => {
        if (!searchTerm.trim()) {
            showSnackbar(
                "Nothing to search. You can use only use order ID",
                "warning"
            );
            return;
        }
                    ...
                    <Stack gap={2}>
                        <Stack>
                            ...
                            <Typography variant="body2" color="gray">
                                Manage and track all customer orders and
                                purchase activities
                            </Typography>
                        </Stack>
                        <Stack gap={2}>
                                ...
                                <Stack gap={2}>
                                    <Stack>
                                        ...
                                        <Typography
                                            ...
                                            color="gray"
                                        >
                                            Browse all orders by their current
                                            status and type
                                        </Typography>
                                    </Stack>
                                    <OrderList
