...
import { ChevronRightIcon, PackageSearch, PackageSearchIcon } from 'lucide-react'
...
const AuctionProductDetails = ({productId, onClose, auction}) => {
    ...
    return (
        <>
            <Stack gap={3} sx={{mt: 2}}>
                <Stack>
                    <Typography variant="subtitle2" color="secondary">Auctin and Product Info</Typography>
                    <Typography variant="body2" color="gray">Comprehensive overview of the item details and auction parameters</Typography>
                </Stack>
                <Box
                ...
                >
                    <Button sx={{py: 2}} onClick={handleDetailsOpen} fullWidth>
                        <Stack width={'100%'} alignItems={'center'} justifyContent={'space-between'} direction={'row'}>
                            <Stack alignItems={'center'} gap={1} direction={'row'}>
                                <PackageSearchIcon/>
                                <Typography variant="subtitle2" color="secondary">{auction.inventoryId?.productName}</Typography>
                            </Stack>
                            <ChevronRightIcon/>
                        </Stack>
                    </Button>
                ...
                <Box>
                    ...
                    {auction.status === 'CLOSED' &&
                        <ClosedDetails data={auction}/>
                    }
                </Box>
            </Stack>
            <ProductDetails open={open} onClose={handleDetailsClose} product={auction.inventoryId}/>
        </>
    )
}
