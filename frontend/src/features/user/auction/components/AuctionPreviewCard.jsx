...
export default function AuctionPreviewCard({
    ...
    useEffect(() => {
        const fetchBidders = async () => {
            const bidders = await getBidders(product._id, user.token);
            const bid = bidders.find((b) => b?.userId?._id === user.userId);
            console.log("user bidders", bid);
            setUserBid(bid);
        };
    ...
    return (
        <Box>
            ...
            <Stack gap={2} sx={{ p: 1, boxShadow: 5, borderRadius: 2 }}>
                ...
                <Stack gap={2} sx={{ px: 1 }}>
                    <Stack>
                        <Typography variant="subtitle1" gutterBottom>
                            {product.inventoryId?.productName || "Unnamed Item"}
                        </Typography>
                        <Stack direction={"row"}>
                            <Typography
                                variant="body2"
                            ...
                            >
                                {toTitleCase(product.inventoryId?.condition)}
                            </Typography>
                        </Stack>
                    </Stack>
                ...
                <Button
                    disabled={
                        new Date(product.endTime) - new Date() >= new Date()
                    }
                    component={Link}
                    to={
                        userBid?.amount > 0
                            ? `/auction/listing/details/${product._id}`
                            : `/auction/listing/product-preview/${product._id}`
                    }
                    variant={
                        userBid?.amount > 0
