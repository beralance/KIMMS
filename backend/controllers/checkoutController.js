...
export const handleCheckout = async (req, res) => {
    try {
        ...
        const cart = await Cart.findOne({ userId }).populate(
            "items.productId",
            "images productName condition price"
        );
        ...
        if (invalidItems.length > 0) {
            return res.status(403).json({
                message:
                    "Your cart containes products not available in your region",
                blockedProducts: invalidItems.map(
                    (i) => i.productId?.productName
                ),
            });
        }
        ...
        if (productIds.length === 0) {
            return res
                .status(400)
                .json({ message: "No valid products in cart" });
        }
...
export const handleCodCheckout = async (req, res) => {
    try {
        const { userId, products, totalPrice, finalPrice, orderType } =
            req.body;
        ...
        if (user.auctionRestriction) {
            console.log(
                `User ${user.fullName} has BAD RECORD and cannot proceed using CASH ON DELIVERY payment method.`
            );
            return res.status(403).json({
                message:
                    "You cannot checkout using Cash on Delivery due to recent bad activities. Please use GCash instead.",
            });
        }
        if (!userId) {
